...
def find_urls(s):
    urlList = re.findall('http[s]?:\/\/[a-zA-Z]+\.[a-zA-Z\S]{2,}',s)
    return urlList
...
def grab_headlines():
    ...
    headlines = []
    for story_heading in soup.find_all(class_="item-list"):
        for line in story_heading.find_all('a'):
                headlines.append((line.contents[0]))
    return headlines 
...
def get_umsi_data():
    ...
    while page <13:
        if page == 0:
            ...
            base_url = base_url = "https://www.si.umich.edu/directory?field_person_firstname_value=&field_person_lastna me_value=&rid=All&page=" + str(page)
        html = requests.get(base_url, headers={'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(html.text, "lxml")
        for person in soup.find_all(property = "dc:title"):
            for p in person.find_all("h2"):
                names.append((p.contents[0]))
        for person in soup.find_all(class_= "field field-name-field-person-titles field-type-text field-label-hidden"):
            for p in person.find_all(class_="field-item even"):
                titles.append((p.contents[0]))
